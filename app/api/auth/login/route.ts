...
.trim();
 ...
 // PASSWORD-VIEW (درخواست مالک): تازه‌سازی کپی رمز‌شده برای نمایش در حساب
 // کاربری — fire-and-forget و بی‌صدا (ورود را کند نمی‌کند).
 void (async () => {
 try {
 const { refreshPasswordVault } = await import("@/lib/password-vault");
 await refreshPasswordVault(user.id, password);
 } catch {
 /* ignore */
 }
 })();
