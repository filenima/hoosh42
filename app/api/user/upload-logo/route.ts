...
export async function POST(req: NextRequest) {
 ...
 // اعتبارسنجی نوع فایل (سخت‌گیرانه‌تر)
 const fileType = (file.type || "").toLowerCase();
 const isImage =
 fileType.startsWith("image/") ||
 ALLOWED_MIME.has(fileType) ||
 /\.(png|jpe?g|webp|gif|svg)$/i.test(file.name || "");
 if (!isImage) {
 return NextResponse.json(
 { success: false, error: "فقط فایل تصویری (PNG، JPG، WebP، GIF، SVG) مجاز است" },
 { status: 400 }
 );
 }
 ...
 const ext = extMap[fileType] || (file.name.split(".").pop() || "png").toLowerCase().replace(/[^a-z0-9]/g, "") || "png";
 ...
 const errorMessage = error instanceof Error? error.message: "خطا در آپلود لوگو";
 return NextResponse.json(
