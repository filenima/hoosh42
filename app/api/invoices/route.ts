...
export async function POST(req: NextRequest) {
  try {
    ...
    // FUNNEL-AB (درخواست مالک): «first_invoice» — اولین فاکتورِ ثبت‌شدهٔ کاربر
    // در قیف تبدیل A/B (fire-and-forget؛ رزرو هم فعالیت واقعی است).
    try {
      const { trackFunnelStep } = await import("@/lib/funnel-ab");
      void trackFunnelStep(userId ?? "", "first_invoice").catch(() => undefined);
    } catch {
      /* ignore */
    }
