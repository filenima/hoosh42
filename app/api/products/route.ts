...
export async function GET(req: NextRequest) {
 ...
 // PERF-1200: خلاصهٔ آماری برای کارت‌های KPI انبار (بدون نیاز به واکشی همهٔ کالاها)
 const withSummary = searchParams.get("withSummary") === "1";
 ...
 // PERF-1200: خلاصهٔ آماری کل انبار (tenant-wide) — فقط با withSummary=1
 // تا فراخوانی‌های سبک (انتخاب کالا در فاکتور) هزینهٔ اضافه نپردازند.
 let summary: Record<string, number> | undefined;
 if (withSummary) {
 const stockAggAll = await db.stockItem.groupBy({
 by: ["productId"],
 where: { tenantId },
 _sum: { quantity: true },
 });
 const stockByProduct = new Map(
 stockAggAll.map((s) => [s.productId, s._sum.quantity ?? 0])
 );
 const pricedProducts = await db.product.findMany({
 where: { tenantId, deletedAt: null },
 select: { id: true, purchasePrice: true, salePrice: true, minStock: true },
 });
 let totalValue = 0;
 let lowStock = 0;
 let zeroStock = 0;
 ...
 inStock++;
 totalValue += Number(
 (p.purchasePrice ?? BigInt(0)) > BigInt(0)
 ? (p.purchasePrice as bigint) * BigInt(Math.trunc(qty))
 : (p.salePrice as bigint) * BigInt(Math.trunc(qty))
 );
 if (p.minStock > 0 && qty <= p.minStock) lowStock++;
 else if (p.minStock === 0 && qty <= 5) lowStock++;
 }
 summary = {
 ...
 totalStockValueRial: totalValue,
 totalStockValueToman: Math.trunc(totalValue / 10),
 };
 }
...
export async function POST(req: NextRequest) {
 ...
 taxRate: Number(parsed.data.taxRate) || 0,
    // EDIT-ALL: شناسه کالا/خدمت مودیان از فرم
    goodsCode: parsed.data.goodsCode || null,
...
export async function PATCH(req: NextRequest) {
 try {
 const ip = getClientIp(req);
 const rl = rateLimitCheck(`product-bulk:${ip}`, 10, 60_000);
 if (!rl.ok) {
 return NextResponse.json(
 ...
 if (ids.length > 100) {
 return NextResponse.json(
 { success: false, error: "حداکثر ۱۰۰ کالا در هر درخواست" },
 { status: 400 }
 );
 }
