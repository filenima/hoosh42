...
...config.optimization,
 ...
 ? {}
 : {
 // FIX(sandbox-OOM): سقف حافظهٔ Turbopack ۳GB → ۱۵۳۶MB کاهش یافت —
 // کرنل چند بار next-server را با OOM کشت (مرورگر QA + پیک کامپایل
 // چانک‌ها روی ۴GB کل). سقف پایین‌تر GC تهاجمی‌تر و جا برای مرورگر می‌گذارد.
 turbopackMemoryLimit: 1536,
 // FIX(QA-warm-loop): کش فایل‌سیستم Turbopack روشن شد — کامپایل‌ها پس از
 // ری‌استارت سرور روی دیسک می‌مانند تا چرخهٔ گرم‌سازی مرورگر همگرا شود
 // (بدون این، هر ری‌استارت OOM کل کامپایل را از نو شروع می‌کرد و سرور
 // دوباره کشته می‌شد). هزینه: ~200MB file-rss — با سقف ۱۵۳۶ جا می‌شود.
