...
// FIX: ConsentBanner حذف شد (درخواست مالک) — بنر GDPR در پنل کاربر مزاحم بود.
...
// VOICE-REMOVE (درخواست مالک): فرمان صوتی شناور حذف شد — Web Speech API در
// ایران مسدود است و ویجت هرگز کار نمی‌کرد؛ ناوبری از سایدبار + CommandPalette (Ctrl+K).
...
const NAV_ITEMS: NavItem[] = [
 ...
 // FIX(v12.1 — سیستم رفرال): ویجت دعوت دوستان که قبلاً کد مرده بود، حالا
 // ماژول رسمی سایدبار است
 { id: "referral", label: "دعوت دوستان", icon: Users, group: "سیستم", badge: "پاداش" },
...
const MODULE_TITLES: Record<string, { title: string; subtitle: string }> = {
 ...
 referral: { title: "دعوت دوستان", subtitle: "کد دعوت اختصاصی، پاداش ۵۰,۰۰۰ تومانی و امتیاز وفاداری" },
...
: `/${view}`;
 ...
 // PERF: پیش‌بارگذاری چانک ماژول‌های پرکاربرد در زمان بیکاری مرورگر —
 // کلیک روی هر ماژول بلافاصله باز می‌شود بدون مکث کامپایل.
 React.useEffect(() => {
 if (view!== "app" ||!userToken) return;
 const preload = () => {
 // ماژول‌های پرتکرار کسب‌وکارهای کوچک
 import("@/components/modules/pos-terminal").catch(() => {});
 import("@/components/modules/quick-invoice").catch(() => {});
 import("@/components/modules/quick-expense").catch(() => {});
 import("@/components/modules/end-of-day").catch(() => {});
 ...
 const meta = MODULE_TITLES[activeModule] || { title: "هوش", subtitle: "" };
 ...
 <aside
 data-tour="sidebar"
 className={`hidden lg:flex shrink-0 flex-col border-e border-border bg-sidebar sticky top-0 h-screen transition-all duration-200 ${
 ...
 className="lg:hidden fixed start-0 top-0 z-50 h-full w-[85vw] max-w-xs bg-sidebar border-e border-border flex flex-col shadow-2xl"
 role="dialog"
...
: "مهمان هوش"}
 ...
 {/* FIX: بنر رضایت کوکی (GDPR) حذف شد — درخواست مالک: برای کاربران ایرانی
 زائد بود و در پنل کاربر مزاحم. getConsent() در lib/marketing.ts پیش‌فرض
 «پذیرش همه» برمی‌گرداند؛ اسکریپت‌های تحلیلی بدون پرسش کار می‌کنند. */}
...
function SidebarNav({
 ...
 <span className="flex-1 text-start">{group}</span>
