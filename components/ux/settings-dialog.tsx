...
import { ACCENT_OPTIONS, getStoredAccentId, setAccent } from "@/lib/accent-colors";
import {
 Dialog,
 DialogContent,
 DialogDescription,
...
export function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
 ...
 // ACCENT: رنگ تم انتخابی — بلافاصله روی CSS vars اعمال می‌شود
 const [accentId, setAccentId] = React.useState<string>("indigo");
 React.useEffect(() => {
 const stored = getStoredAccentId();
 ...
 <DialogContent className="max-w-3xl max-h-[88vh] p-0 overflow-hidden gap-0">
...
: "bg-muted text-muted-foreground"
 ...
 {/* ----- رنگ تم (ACCENT — درخواست مالک: رنگ باید واقعاً اعمال شود) ----- */}
 <div className="space-y-2">
 <div className="flex items-center gap-1.5">
 <Palette className="h-3.5 w-3.5 text-primary" />
 <h3 className="text-sm font-semibold">رنگ تم</h3>
 </div>
 <p className="text-xs text-muted-foreground">
 رنگ اصلی پنل، دکمه‌ها و آیکون‌ها — بلافاصله اعمال می‌شود
 </p>
 ...
 key={opt.id}
 option={opt}
 active={accentId === opt.id}
 onPick={() => {
 setAccent(opt.id);
 setAccentId(opt.id);
 toast({
 title: "رنگ تم اعمال شد",
 description: `رنگ «${opt.label}» روی کل پنل اعمال شد.`,
 });
 }}
 />
 ))}
...
/**
 * AccentSwatch — سواچ رنگ تم (ACCENT): انتخاب رنگ واقعی پنل/آیکون‌ها.
 * پیش‌نمایش زنده: دکمه + آیکون هم‌رنگ گزینه، حلقهٔ انتخاب هنگام فعال بودن.
 */
function AccentSwatch({
 ...
 option: { id: string; label: string; light: string; dark: string };
 active: boolean;
 onPick: () => void;
}) {
 return (
 ...
 title={option.label}
 aria-pressed={active}
 aria-label={`رنگ تم ${option.label}`}
