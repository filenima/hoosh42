...
const SOURCES: SourceOption[] = [
 ...
 { key: "excel", label: "Excel / CSV", desc: "فایل گسترده", icon: FileSpreadsheet, extensions: ".csv,.xlsx,.xls" },
 { key: "other", label: "سایر", desc: "سایر فرمت‌ها", icon: FileSpreadsheet, extensions: ".csv,.xlsx,.zip" },
];
...
const AUTO_MAPPING: Record<string, string> = {
 ...
 "کد کالا": "Product.sku", "شناسه کالا": "Product.sku", "کد شناسایی": "Product.sku", "کد یکتا": "Product.sku",
 "بارکد": "Product.barcode", "کد بارکد": "Product.barcode",
 "واحد شمارش": "Product.unit", "واحد": "Product.unit", "واحد کالا": "Product.unit", "آیتم": "Product.unit",
 "دسته‌بندی": "Product.category", "گروه کالا": "Product.category", "گروه": "Product.category",
 "قیمت فروش": "Product.salePrice", "قیمت": "Product.salePrice", "نرخ فروش": "Product.salePrice",
 "قیمت خرید": "Product.purchasePrice", "نرخ خرید": "Product.purchasePrice",
...
export function MigrationWizard({ open, onOpenChange }: MigrationWizardProps) {
 ...
 const parseFile = React.useCallback(async (f: File): Promise<{ headers: string[]; rows: PreviewRow[] }> => {
  ...
  if (isXlsx) {
    ...
    // CSV واقعی — پارسر کامل با کوتیشن + تشخیص خودکار جداکننده (،/;/Tab)
    ...
    if (filtered.length === 0) throw new Error("فایل CSV خالی است");
    hs = filtered[0].map((h) => h.trim());
    // FIX(هدر تکراری): همانند XLSX — ستون هم‌نام دادهٔ ستون قبلی را بازنویسی می‌کرد
    dedupeHeaders(hs);
    all = filtered.slice(1).map((r) => {
      const rowObj: PreviewRow = {};
      hs.forEach((h, i) => {
        rowObj[h || "ستون " + String(i + 1)] = (r[i] ?? "").trim();
      });
      return rowObj;
    });
  } else {
  ...
  if (all.length > 5000) {
    throw new Error("فایل بزرگ است (" + all.length + " ردیف) — حداکثر ۵۰۰۰ ردیف. فایل را بخش‌بندی کنید.");
  }
