 ...
 *
 * دیالوگ ۴ مرحله‌ای:
 * 1) دانلود قالب + آپلود فایل CSV (drag-and-drop)
 ...
 *
 * پارس ساده‌ی CSV (پشتیبانی از quote و کاما داخل فیلد) — بدون کتابخانه‌ی خارجی.
 */
...
// پارسر ساده‌ی CSV — پشتیبانی از quote و کاما داخل فیلد
...
: 1;
 ...
 toast({
 title: "فایل خالی است",
 description: "هیچ ردیفی در فایل CSV یافت نشد.",
 variant: "destructive",
 });
 setParsedRows([]);
 return;
 ...
 toast({
 title: "فایل پارس شد",
 description: `${toPersianDigits(rows.length)} ردیف آماده‌ی وارد کردن است.`,
 });
 } catch (err) {
 toast({
 title: "خطا در پارس کردن CSV",
...
: ""
 ...
 فایل CSV را با ستون‌های مشخص‌شده آپلود کنید. می‌توانید قالب نمونه را دانلود کنید.
 </DialogDescription>
 </DialogHeader>
...
: "bg-muted text-muted-foreground"
 ...
 فایل CSV با ستون‌های مشخص‌شده آپلود کنید
 </p>
 </div>
 <Button
 type="button"
 ...
 accept=".csv,text/csv"
 onFile={handleFile}
 label="فایل CSV را اینجا رها کنید یا کلیک کنید"
 hint={`ستون‌ها: ${columns.map((c) => c.key).join("، ")}`}
 maxSize={5}
 />
 )}
