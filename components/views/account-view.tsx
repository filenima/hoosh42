...
export function AccountView({ token, onBack }: AccountViewProps) {
 ...
 // PASSWORD-VIEW (درخواست مالک): نمایش/کپی رمز عبور خود کاربر
 ...
 if (json?.success && typeof json.password === "string") {
 setRevealedPassword(json.password);
 } else {
 setRevealError(
 json?.error ||
 "نمایش رمز در دسترس نیست — یک بار خارج شده و دوباره وارد شوید."
 );
 }
 } catch {
 setRevealError("خطا در نمایش رمز عبور — دوباره تلاش کنید.");
 } finally {
...
: 0;
 ...
 {/* نمایش رمز عبور — قابل مشاهده و کپی (PASSWORD-VIEW — درخواست مالک) */}
 <div className="space-y-1.5">
 <Label className="text-xs flex items-center justify-between">
 <span>رمز عبور</span>
 <Badge variant="outline" className="text-[9px] text-success border-success/30 bg-success/5">
 ...
 <Input
 type={revealedPassword !== null? "text": "password"}
 value={revealedPassword?? "••••••••"}
 readOnly
 className="ps-9 pe-9 h-10 bg-muted/40 text-sm font-mono"
 dir="ltr"
 />
 </div>
 <Button
 ...
 aria-label={revealedPassword !== null? "پنهان کردن رمز": "نمایش رمز"}
 >
 {revealLoading? (
 ...
 {revealLoading? "در حال دریافت...": revealedPassword !== null? "پنهان": "نمایش رمز"}
 </span>
 </Button>
 {revealedPassword !== null && (
 <Button
 ...
 {revealError? (
 <p className="text-[10px] text-warning leading-relaxed">{revealError}</p>
 ): revealedPassword !== null? (
 <p className="text-[10px] text-muted-foreground">
 این رمز عبور فعلی حساب شماست — با دکمهٔ کناری کپی کنید. پس از تغییر رمز، همین‌جا به‌روز می‌شود.
 </p>
 ): (
 <p className="text-[10px] text-muted-foreground">
 روی «نمایش رمز» بزنید تا رمز عبور خود را ببینید و کپی کنید.
 </p>
 )}
 </div>
