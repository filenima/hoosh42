...
// FIX(RACE): صفِ سریالیزهٔ in-process — رویدادهای هم‌زمان (signup +
// trial_started در یک ثبت‌نام) قبلاً snapshot یکسان می‌خواندند و write آخر
// بقیه را بازنویسی می‌کرد. زنجیرهٔ promise خواندن-تغییر-نوشتن را ترتیبی می‌کند.
...
export async function trackFunnelStep(
 ...
 if (!userId) return;
 // FIX(RACE): فراخوانی‌های هم‌زمان (مثل signup+trial_started در ثبت‌نام)
 // هرکدام snapshot یکسانی می‌خواندند و آخرین write بقیه را بازنویسی می‌کرد
 // (رویدادها گم می‌شدند). صفِ in-process خواندن-و-نوشتن را سریالیزه می‌کند.
 const run = funnelQueue.then(async () => {
 ...
 // صف را زنجیر می‌کنیم؛ خطای یکی صف را نمی‌شکند
 funnelQueue = run.catch(() => undefined);
 return run;
}
